...
<header class="fixed top-0 left-0 right-0 h-16 bg-[#0f172a]/80 backdrop-blur-xl border-b border-white/5 z-50 shadow-2xl">
    ...
    <div class="flex-1 max-w-sm md:max-w-md ml-4 md:ml-6 relative">
        ...
        <div class="hidden md:flex items-center gap-2">
            ...
            <abbr title="Home">
                <a href="<?php echo e(route('home')); ?>" class="load-link relative inline-flex items-center p-2 text-slate-400 hover:text-blue-400 hover:bg-white/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <abbr title="Messages">
                ...
                <?php if(auth()->guard()->check()): ?>
                    ...
                    <span id="message-badge"
                            class="absolute -top-1 -right-1 bg-blue-600 text-white text-[10px] font-black min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full border-2 border-[#0f172a] animate-pulse <?php echo e($msgCount > 0 ? '' : 'hidden'); ?>">
                        <?php echo e($msgCount > 9 ? '9+' : $msgCount); ?>
            ...
            <abbr title="Newsfeed">
                <a href="<?php echo e(route('newsfeed')); ?>" class="load-link relative inline-flex items-center p-2 text-slate-400 hover:text-blue-400 hover:bg-white/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <abbr title="Notifications">
                <a href="<?php echo e(route('notifications.index')); ?>" class="load-link relative inline-flex items-center p-2 text-slate-400 hover:text-blue-400 transition">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        ...
                        <span id="notification-badge"
                                class="notif-badge-count absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-black min-w-[18px] h-[18px] flex items-center justify-center rounded-full animate-pulse border-2 border-[#0f172a] <?php echo e($count > 0 ? '' : 'hidden'); ?>">
            ...
            <abbr title="Account Settings">
                <a href="<?php echo e(route('settings.edit')); ?>" class="load-link text-slate-400 hover:text-blue-400 transition">
            ...
            <div class="flex items-center gap-3 ml-2 border-l border-white/10 pl-4">
                ...
                <abbr title="Profile">
                    <a href="<?php echo e(route('profile')); ?>" class="load-link">
    ...
    <div id="mobileMenu" class="load-link hidden fixed top-20 right-8 bg-[#1e293b]/95 backdrop-blur-2xl text-white z-40 shadow-2xl rounded border border-white/10 p-2">
        <nav class="flex flex-col gap-2">
            <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-3 p-2 hover:bg-white/5 rounded-2xl transition">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M13 5v6h6m-6 0H7v6h6v6h6v-6h-6V5z"/>
                </svg>
            </a>

            <a href="<?php echo e(route('messages.index')); ?>" class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition">
                <div class="relative">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        <?php $mCount = Auth::user()->receivedMessages()->whereNull('read_at')->count(); ?>
                        <?php if($mCount > 0): ?>
            ...
            <a href="<?php echo e(route('newsfeed')); ?>" class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h10M4 18h10"/>
                </svg>
            </a>

            <a href="<?php echo e(route('notifications.index')); ?>" class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition group">
                <div class="relative">
                    ...
                    <?php $unreadNotifs = Auth::user()->unreadNotifications->count(); ?>
                    <?php if($unreadNotifs > 0): ?>
                        <span class="notif-badge-count absolute -top-2 -right-2 bg-red-600 text-white text-[8px] font-black min-w-[15px] h-[15px] flex items-center justify-center rounded-full animate-pulse border-2 border-[#0f172a] shadow-lg">
                            <?php echo e($unreadNotifs > 9 ? '9+' : $unreadNotifs); ?>
            ...
            <a href="<?php echo e(route('settings.edit')); ?>" class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition">
                <svg class="h-6 w-6 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                </svg>
            </a>

            <a href="<?php echo e(route('profile')); ?>" class="flex items-center gap-3 p-3 hover:bg-white/5 rounded-2xl transition">
                <img src="<?php echo e(auth()->user()->profile_picture_url); ?>"
                                            alt="<?php echo e(auth()->user()->name); ?>"
                                            class="h-9 w-9 rounded object-cover border border-white/10 shadow-sm">
...
<script>
    ...
    searchInput?.addEventListener('input', function(e) {
        ...
        debounceTimer = setTimeout(() => {
            fetch(`<?php echo e(url('users/search')); ?>?query=${query}`)
                .then(res => res.json())
                .then(data => {
        ...
        if (userId && window.Echo) {
            window.Echo.private(`App.Models.User.${userId}`)
                .notification((notification) => {
                    ...
                    fetch("<?php echo e(route('notifications.counts')); ?>")
                        .then(res => res.json())
...
<?php /**PATH C:\Users\Kristel Lonzame\Desktop\Website_ojtFinder\htdocs\resources\views/partial/navigation.blade.php ENDPATH**/ ?>